...
def test_oem_filter_survives_events_being_paged_out(populated_db):
    """Regression: the OEM filter dropdown must list every manufacturer
    with real products, not just the manufacturers present in whatever
    slice of change_events happens to fit in the (deliberately bounded,
    docs/ENTERPRISE_OEM_ARCHITECTURE.md's scale note) events window. A
    limit=0 events window simulates a large, more-recent crawl elsewhere
    pushing GMKtec's own events off the visible page."""
    conn = connect_readonly(populated_db)
    data = collect(conn, limit=0)
    conn.close()
    assert data["events"] == []  # the window that would starve a naive filter
    assert any(m["name"] == "GMKtec" for m in data["manufacturers"])
    html = render(data)
    # The OEM dropdown is populated from the single oemRegistry() helper
    # (backed by DATA.manufacturers), never from DATA.events — assert the
    # source wiring, not just the embedded data.
    assert "function oemRegistry()" in html
    assert "const mans=oemRegistry();" in html


def test_evidence_items_appear_in_dashboard_data(tmp_path):
    from oem_radar.core.models import EvidenceItem, EvidenceKind, EvidenceProvenance

    store = SqliteStore(str(tmp_path / "ev.db"), str(tmp_path / "raw"))
    store.record_evidence_item(EvidenceItem(
        manufacturer="Lenovo", source_id="lenovo-psref",
        evidence_kind=EvidenceKind.PRODUCT_DATABASE,
        provenance=EvidenceProvenance.OFFICIAL_PRODUCT_DATABASE,
        canonical_url="https://psref.lenovo.com/Product/ThinkPad/X1",
        external_id="1523", model="ThinkPad X1 Carbon Gen 12",
    ))
    store.close()

    conn = connect_readonly(str(tmp_path / "ev.db"))
    data = collect(conn)
    conn.close()
    assert len(data["evidence_items"]) == 1
    item = data["evidence_items"][0]
    assert item["manufacturer"] == "Lenovo"
    assert item["evidence_kind"] == "product_database"
    assert item["linked_product_key"] is None  # no matching product in this DB

    html = render(data)
    assert "ThinkPad X1 Carbon Gen 12" in html


def test_evidence_items_survive_collect_and_render(populated_db):
    """Regression: Stage 11's evidence_items query must degrade gracefully
    (empty list, not a crash) on a DB with no evidence rows, and the
    dashboard must render the Evidence tab's JS wiring either way."""
    conn = connect_readonly(populated_db)
    data = collect(conn)
    conn.close()
    assert data["evidence_items"] == []  # populated_db has no evidence — real, not fabricated
    html = render(data)
    assert 'data-tab="evidence"' in html
    assert "renderEvidence" in html
