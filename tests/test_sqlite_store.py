...
def test_resolve_prior_distinct_vendor_skus_never_merge(store):
    """Stage 8 regression: Samsung's real catalog has two genuinely
    different products — 'Galaxy Book6 Ultra (16", 64 GB)' and '...(16", 32
    GB)' — that share a model_key (same first digit-bearing token, 'book6')
    and the same tier word ('ultra'), so _same_product() alone would merge
    them. When both listings carry a real, distinct vendor SKU, that's a
    stronger and more specific signal than the coarse name match and must
    win: these are different products, not an update of one another."""
    store.append("src:sku-a", make_product(
        manufacturer="Samsung", model='Galaxy Book6 Ultra (16", 64 GB), a Copilot+ PC',
        vendor_sku="NP960UJH-XG7US"))
    prior, relation = store.resolve_prior(
        "src:sku-b",
        make_product(manufacturer="Samsung",
                     model='Galaxy Book6 Ultra (16", 32 GB), a Copilot+ PC',
                     vendor_sku="NP960UJG-KG2US"),
    )
    assert relation == "none" and prior is None

    # Sanity: the same pair WOULD merge via the coarse path if one side has
    # no SKU at all (the guard only fires when both sides disagree).
    prior, relation = store.resolve_prior(
        "src:sku-c",
        make_product(manufacturer="Samsung",
                     model='Galaxy Book6 Ultra (16", 32 GB), a Copilot+ PC',
