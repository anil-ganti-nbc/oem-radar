...
class DashboardConfig(BaseModel):
    """How the local dashboard behaves when it is opened.

    The dashboard was read-only through Stage 11.1: it never crawled and
    never touched the network. That made "reload" a lie — it re-queried a
    database nothing was updating, so every visit showed the same stale
    runs unless a scheduled task or a terminal had crawled in between.
    These settings let the dashboard start a crawl, and they are settings
    rather than hardcoded behaviour precisely because starting a crawl is
    ...
    #: per-click way to do that.
    auto_crawl_force: bool = False
    #: How old the last successful crawl may be before the UI calls it stale.
    stale_after_hours: float = Field(default=6.0, gt=0)
