...
from .models import EXCLUDE_BASELINE_EVENTS_SQL
...
def compute_summary(conn: sqlite3.Connection, start: str | None = None,
    ...
    # Baseline events (a source's first-ever crawl) are excluded from every
    # signal/noise metric here, same as the dashboard's default view --
    # they are never reviewed, so left in they would just inflate
    # unreviewed_alerts and understate review_completion_rate without
    # meaning anything.
    where = [EXCLUDE_BASELINE_EVENTS_SQL]
    params: list[Any] = []
    if start:
...
def compute_rankings(
    ...
    where = ["r.id IS NULL", EXCLUDE_BASELINE_EVENTS_SQL]
    params: list[Any] = []
    if start:
        where.append("e.detected_at >= ?")
        params.append(start)
    ...
    oldest = conn.execute(
        ...
        f"LEFT JOIN alert_reviews r ON r.alert_id = e.id "
        f"WHERE r.id IS NULL AND {EXCLUDE_BASELINE_EVENTS_SQL} "
