...
class EvidenceSource(Protocol):
    """Stage 11 (Evidence Fusion v0.1): one implementation per alternate
    official evidence surface (a product database, a support/BIOS/driver
    index — never a storefront, that's SourceEngine's job). Deliberately a
    sibling protocol, not a SourceEngine variant — an EvidenceItem is not
    a NormalizedProduct with gaps, see docs/EVIDENCE_ARCHITECTURE.md."""

    config_schema: type  # pydantic model for the YAML/config block, if any
