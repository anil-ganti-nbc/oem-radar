...
_MIGRATIONS: dict[int, list[str]] = {
    ...
    6: [
        # Evidence Fusion v0.1 (Stage 11) — see schema.sql for the fresh-DB
        # version of these same tables.
        "CREATE TABLE IF NOT EXISTS evidence_items ("
        "id INTEGER PRIMARY KEY, source_id TEXT NOT NULL, manufacturer TEXT NOT NULL, "
        "evidence_kind TEXT NOT NULL, provenance TEXT NOT NULL, canonical_url TEXT NOT NULL, "
        "external_id TEXT NOT NULL, model TEXT, sku TEXT, mpn TEXT, family TEXT, title TEXT, "
        "description TEXT, version TEXT, filename TEXT, region TEXT, published_at TEXT, "
        "observed_at TEXT NOT NULL DEFAULT (datetime('now')), confidence REAL NOT NULL DEFAULT 1.0, "
        "content_hash TEXT NOT NULL, raw_data_json TEXT NOT NULL DEFAULT '{}', "
        ...
        "id INTEGER PRIMARY KEY, evidence_item_id INTEGER NOT NULL REFERENCES evidence_items(id), "
        "product_key TEXT, method TEXT NOT NULL, confidence REAL NOT NULL DEFAULT 1.0, "
        "created_at TEXT NOT NULL DEFAULT (datetime('now')), "
        "UNIQUE(evidence_item_id, product_key))",
        "CREATE INDEX IF NOT EXISTS idx_evidence_links_product ON evidence_links(product_key)",
    ],
    7: [
        # Stage 11.1 — undo Stage 11's evidence-as-product-alert shortcut.
        # Evidence observations get their own log; change_events goes back to
        # meaning exactly one thing: a tracked product changed.
        ...
        # Backfill: MOVE (not discard) any evidence rows Stage 11 wrote into
        # change_events. product_key is 'evidence:<source_id>:<external_id>';
        # 'evidence:' is 9 chars, so substr(...,10) is the item's natural key.
        "INSERT INTO evidence_events(evidence_item_id, event_type, detected_at, meta_json) "
        "SELECT ei.id, "
        "CASE WHEN e.change_type='support_artifact_updated' THEN 'updated' ELSE 'added' END, "
        "e.detected_at, COALESCE(e.meta_json,'{}') "
        "FROM change_events e JOIN evidence_items ei "
        "ON ei.source_id || ':' || ei.external_id = substr(e.product_key, 10) "
        "WHERE e.change_type IN ('support_artifact_added','support_artifact_updated') "
        "AND e.product_key LIKE 'evidence:%'",
        # ...then remove them (and anything hanging off them) from the alert
        # stream. Safe by construction: only rows this project's own Stage 11
        # evidence pipeline could have created match both predicates.
        "DELETE FROM notifications WHERE change_event_id IN "
        "(SELECT id FROM change_events WHERE change_type IN "
        "('support_artifact_added','support_artifact_updated') AND product_key LIKE 'evidence:%')",
        "DELETE FROM alert_review_history WHERE alert_id IN "
        "(SELECT id FROM change_events WHERE change_type IN "
        "('support_artifact_added','support_artifact_updated') AND product_key LIKE 'evidence:%')",
        "DELETE FROM alert_reviews WHERE alert_id IN "
        "(SELECT id FROM change_events WHERE change_type IN "
        "('support_artifact_added','support_artifact_updated') AND product_key LIKE 'evidence:%')",
        "DELETE FROM change_events WHERE change_type IN "
        "('support_artifact_added','support_artifact_updated') AND product_key LIKE 'evidence:%'",
    ],
}
...
class SqliteStore:
    ...
    def ensure_manufacturer(self, name: str, country: str | None, aliases: list[str]) -> int:
        ...
        # Commit here, not at some later pipeline boundary: registering an
        # OEM is independent of any crawl transaction, and `sync_oem_registry`
        # does nothing but call this before closing the store. Without the
        # commit those upserts are rolled back on close and the manufacturer
        # filter silently stays incomplete — exactly the Stage 11.1 bug.
        self.db.commit()
    ...
    def resolve_prior(self, product_key: str, product: NormalizedProduct):
        ...
        if row:
            ...
            if snap:
                ...
                # Both sides carry a real, distinct vendor SKU: that's a
                # stronger, more specific signal than the coarse model_key
                # match and it disagrees, so these are different products
                # (e.g. Samsung's "Galaxy Book6 Ultra (16", 64 GB)" vs
                # "...(16", 32 GB)" share a model_key and both carry the
                # "ultra" tier word, but are different SKUs/configs sold as
                # separate listings, not a rename/update of one another).
                if (product.vendor_sku and cand.vendor_sku
                        and product.vendor_sku != cand.vendor_sku):
                    return None, "none"
    ...
    # -- evidence (Stage 11, Evidence Fusion v0.1) ----------------------------

    def record_evidence_item(self, item) -> tuple[int, str]:
        """Insert-or-update an EvidenceItem, deduped on (source_id,
        external_id). Returns (row id, 'new'|'updated'|'unchanged') — the
        content hash decides 'updated' vs 'unchanged' so a re-observation
        ...
        if existing is None:
            cur = self.db.execute(
                ...
                "canonical_url, external_id, model, sku, mpn, family, title, description, version, "
                "filename, region, published_at, observed_at, confidence, content_hash, raw_data_json) "
                "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (item.source_id, item.manufacturer, item.evidence_kind.value, item.provenance.value,
                 item.canonical_url, item.external_id, item.model, item.sku, item.mpn, item.family,
                 item.title, item.description, item.version, item.filename, item.region,
                 published, item.observed_at.isoformat(), item.confidence, content_hash, raw_json),
            )
            self.db.commit()
        ...
        self.db.execute(
            "UPDATE evidence_items SET manufacturer=?, evidence_kind=?, provenance=?, canonical_url=?, "
            "model=?, sku=?, mpn=?, family=?, title=?, description=?, version=?, filename=?, region=?, "
            "published_at=?, observed_at=?, confidence=?, content_hash=?, raw_data_json=? WHERE id=?",
            (item.manufacturer, item.evidence_kind.value, item.provenance.value, item.canonical_url,
             item.model, item.sku, item.mpn, item.family, item.title, item.description, item.version,
             item.filename, item.region, published, item.observed_at.isoformat(), item.confidence,
             content_hash, raw_json, existing["id"]),
        )
        self.db.commit()
        return existing["id"], "updated"

    def resolve_evidence_link(self, item) -> tuple[str | None, str]:
        """Conservative identity correlation only — exact SKU, exact MPN,
        exact model match, or explicit alias, in that order. No fuzzy
        matching (Stage 11: 'not in v0.1'). Returns (product_key or None,
        method) — None means deliberately left unlinked, not a failure."""
        man_row = self.db.execute(
            "SELECT id FROM manufacturers WHERE name=?", (item.manufacturer,)
        ).fetchone()
        if man_row is None:
            return None, "none"
        man_id = man_row["id"]

        for value, method in ((item.sku, "exact_sku"), (item.mpn, "exact_mpn")):
            if not value:
                continue
            row = self.db.execute(
                "SELECT l.product_key FROM listings l JOIN products p ON p.id=l.product_id "
                "WHERE p.manufacturer_id=? AND l.vendor_sku=? ORDER BY l.last_seen_at DESC LIMIT 1",
                (man_id, value),
            ).fetchone()
            if row:
                return row["product_key"], method

        if item.model:
            norm = item.model.strip().lower()
            # Deliberately NOT products.canonical_model here: that column
            # stores model_key()'s coarse key (tokens up to the first
            # digit-bearing one), built for surfacing candidates in
            # resolve_prior — it collapses genuinely different products
            # ("ThinkPad X1 Carbon Gen 12" and "ThinkPad X1 Yoga Gen 9" both
            # key to "thinkpad-x1"). Evidence linking needs to be stricter
            # than that, so this compares the real, exact model string from
            # each listing's latest snapshot instead — bounded by this
            # manufacturer's real listing count, fine at this project's scale.
            listing_rows = self.db.execute(
                "SELECT l.id, l.product_key FROM listings l JOIN products p ON p.id=l.product_id "
                "WHERE p.manufacturer_id=?", (man_id,),
            ).fetchall()
            for lrow in listing_rows:
                snap = self._latest_snapshot_row(lrow["id"])
                if snap is None:
                    continue
                candidate = product_from_snapshot_row(snap)
                if candidate.model.strip().lower() == norm:
                    return lrow["product_key"], "exact_model_id"
            alias_row = self.db.execute(
                "SELECT product_id FROM aliases a JOIN products p ON p.id=a.product_id "
                "WHERE p.manufacturer_id=? AND LOWER(a.alias)=?", (man_id, norm),
            ).fetchone()
            if alias_row:
                listing = self.db.execute(
                    "SELECT product_key FROM listings WHERE product_id=? ORDER BY last_seen_at DESC LIMIT 1",
                    (alias_row["product_id"],),
                ).fetchone()
                if listing:
                    return listing["product_key"], "alias"
        return None, "none"

    def link_evidence(self, evidence_item_id: int, product_key: str | None,
                       method: str, confidence: float) -> None:
        self.db.execute(
            "INSERT OR IGNORE INTO evidence_links(evidence_item_id, product_key, method, confidence) "
            "VALUES (?,?,?,?)",
            (evidence_item_id, product_key, method, confidence),
        )
        self.db.commit()

    def record_evidence_event(self, evidence_item_id: int, event_type: str,
                              meta: dict | None = None) -> int:
        """Log that an evidence source observed a new or changed item.

        Stage 11.1: deliberately NOT `record_event`. This carries no
        severity, feeds no outbox, and is not reviewable — because an
        evidence observation is not a claim that a tracked product
        changed. See `docs/EVIDENCE_ARCHITECTURE.md`."""
        if event_type not in ("added", "updated"):
            raise ValueError(f"unknown evidence event_type: {event_type!r}")
        cur = self.db.execute(
            "INSERT INTO evidence_events(evidence_item_id, event_type, meta_json) VALUES (?,?,?)",
            (evidence_item_id, event_type, json.dumps(meta or {}, default=str)),
        )
        self.db.commit()
        return cur.lastrowid

    def evidence_timeline(self, manufacturer: str | None = None, limit: int = 200) -> list[sqlite3.Row]:
        """Chronological evidence_items, optionally scoped to one
        manufacturer — the 'relational timeline' Stage 11 asked for
        (no graph visualization needed)."""
        if manufacturer:
            return self.db.execute(
                "SELECT * FROM evidence_items WHERE manufacturer=? "
                "ORDER BY observed_at DESC LIMIT ?", (manufacturer, limit),
            ).fetchall()
        return self.db.execute(
            "SELECT * FROM evidence_items ORDER BY observed_at DESC LIMIT ?", (limit,),
        ).fetchall()
