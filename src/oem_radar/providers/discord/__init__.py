...
def _safe_print(line: str) -> None:
    """print(), but never crash a dry run over a console codepage that can't
    encode a star/emoji glyph (observed on Windows cp1252). Falls back to
    replacing unencodable characters rather than losing the whole line."""
    try:
        print(line)
...
class ConsoleNotifier:
    ...
    def enqueue(self, event: ChangeEvent, product: NormalizedProduct | None = None) -> None:
        ...
        if int(event.severity) >= self.min_severity:
            ...
            _safe_print(f"  [{stars(int(event.severity))}] {event.change_type.value}: {name}"
                       + (f" | {event.field}: {event.old_value} → {event.new_value}"
                          if event.field and event.old_value is not None else ""))

    def enqueue_story(self, story) -> None:
        _safe_print(f"  [STORY {story.score}/100] {story.title}")
